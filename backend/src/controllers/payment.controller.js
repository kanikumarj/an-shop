...
exports.uploadScreenshot = async (req, res) => {
  ...
  if (!utrValidation.valid) {
    await deleteFromCloudinary(req.file.path).catch(() => {});
    throw AppError.badRequest(utrValidation.error, 'INVALID_UTR');
  }

  // ── 3. Fraud: Check duplicate UTR ─────────────────────────
  const dupCheck = await checkDuplicateUtr(utrValidation.cleaned);
